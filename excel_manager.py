...
class ExcelManager:
    ...
    def update_payment_status(self, invoice_number, payment_status, payment_date=None):
        ...
        try:
            ...
            mask = df["Invoice Number"] == invoice_number
            if not mask.any():
            ...
            if payment_date:
                df.loc[mask, "Payment Date"] = payment_date
            else:
                df.loc[mask, "Payment Date"] = datetime.now().strftime("%Y-%m-%d")
            
            # Save updated data
            df.to_excel(self.output_file, index=False)
            print(f"✓ Updated payment status for invoice {invoice_number}")
            return True
